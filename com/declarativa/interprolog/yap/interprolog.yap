...
socket_get0(Sockfd,C) :- get_code(Sockfd,_C), socket_get0(_C,Sockfd,C).
...
% socket_put(Sockfd,C)  :- (C >127 -> _C is C-256 ; _C=C), put_code(Sockfd,_C).
...
atomRead(Atom,Term) :- catch((atom_chars(Atom, Chars),append(Chars,[46],NChars),read_from_chars(NChars, Term)),_E,Term=end_of_file).
...
ipInterrupter(InStream,Main) :- 
	...
	!,
	% ipProgressMessage('will now throw something to Prolog thread'-Main),
	% assert(ipYAPthrowHack),
